  ...
  reducers: {
    filterType(state, action) {
      ...
      if (filterSelected !== "all") {
        state.dataFilter = state.data.filter((category) => category.category === filterSelected);
      }else{
        state.dataFilter = state.data
      }

      // state.data = state.data.filter( (filterSelected) => filterSelected === state.data.category )
    },
  },
});

export const listMenuAction = listMenuSlice.actions;

export default listMenuSlice;
